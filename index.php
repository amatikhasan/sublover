...
<div class="container mt-3">
    ...
    $result = $conn->query("SELECT * FROM subtitles LIMIT $start, $limit");
    ...
    <div class="m-2">
        <?php
        $total_result = $conn->query("SELECT COUNT(id) AS id FROM subtitles")->fetch_assoc()['id'];
        ...
        echo "<li class='page-item " . ($current_page == 1 ? 'disabled' : '') . "'><a class='page-link' href='index.php?page=1'>&laquo;</a></li>";
        // Previous Button
        $prev_page = $current_page - 1;
        echo "<li class='page-item " . ($current_page == 1 ? 'disabled' : '') . "'><a class='page-link' href='index.php?page=" . max($prev_page, 1) . "'>&lt;</a></li>";
        ...
        for ($i = $start; $i <= $end; $i++) {
            echo "<li class='page-item " . ($i == $current_page ? 'active' : '') . "'><a class='page-link' href='index.php?page=$i'>$i</a></li>";
        }

        // Next Button
        $next_page = $current_page + 1;
        echo "<li class='page-item " . ($current_page == $total_pages ? 'disabled' : '') . "'><a class='page-link' href='index.php?page=" . min($next_page, $total_pages) . "'>&gt;</a></li>";
        echo "<li class='page-item " . ($current_page == $total_pages ? 'disabled' : '') . "'><a class='page-link' href='index.php?page=$total_pages'>&raquo;</a></li>";
